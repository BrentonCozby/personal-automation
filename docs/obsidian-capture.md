...
`tasks` reads the vault on disk via `OBSIDIAN_VAULT_PATH` (the Mac's synced copy, with no
...
- **Shortcut opens Obsidian but nothing appends.** Almost always (a) `vault=` doesn't
  ...
  Confirm both via "Copy Advanced URI" in the mobile file menu, which shows the correct
  `vault=` and scheme.
