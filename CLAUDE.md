...
- **External services**: each one gets its own folder (`packages/ynab/`, or `apps/ynab-categorize/src/anthropic/` for the Claude API client). Inside: `client.ts` (factory), `schemas.ts` (zod), `types.ts` (derived). Anything specific stays in that folder.
...
- Local runs of categorize: `pnpm test:ynab-categorize` (dry-run + verbose) or `pnpm ynab-categorize`.
