...
// The set of task backends the digest can read from. `obsidian` (Markdown todos in a vault on
// disk) is implemented; `google` (Google Tasks) is an un-implemented placeholder kept to show the
// seam stays open. Keeping this as a const array lets config.ts validate TASK_PROVIDER against it
// without the two drifting apart.
export const TASK_PROVIDERS = ['google', 'obsidian'] as const
...
// rather than as an always-required env var, so a non-Obsidian setup needn't carry a vault path.
