...
const LOCK_PATH = join(tmpdir(), 'ynab-enrich-memos.lock')
...
Options:
  ...
  --lookback-days N     Override ENRICH_LOOKBACK_DAYS env var
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const result = await runEnrich({ config, opts: args })
    console.log(
      `Summary: ${result.succeeded} enriched, ${result.failed} failed, ${result.skipped} skipped`,
    )
