...
const RULES = `Rules:
- A task may include a notes field with context or constraints I've added. Treat notes as authoritative and build the next action around them. For example, if a note says a needed item is already on hand (and where it is), do NOT suggest acquiring it; the next action is to use what I have.
